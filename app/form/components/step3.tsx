...
export default function Step3({ formData, onChange, onNext, onPrev, setFormData, onSubmitData }:
    Step3Props) {
    // สร้าง State เล็กๆ ไว้เก็บสถานะตอนกําลังโหลดข้อมูล
    const [isSubmitting, setIsSubmitting] = useState(false);

    // เปลี่ยนเปน async function เพื่อรอผลลัพธจาก API
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setIsSubmitting(true); // ปรับปุ่มเป็นสถานะกําลังโหลด
        ...
        // ถ้าสําเร็จค่อยเปลี่ยนหน้าไป Step 4
        if (isSuccess) {
            onNext();
        }
    };
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 w-full">
                ...
                {[
                    ...
                    <div key={index} className="border border-slate-300 rounded-md overflow-hidden">
                        <div className="p-3 bg-white">
                            ...
                            <p className="text-sm text-slate-600">จํานวนคงเหลือ: {slot.slots} ช่อง</p>
                        </div>
                        <button
                            type="button"
                            onClick={() => handleTimeSelect(slot.time)}
                            className={`w-full py-2 font-bold ${formData.appointmentTime === slot.time ?
                                'bg-slate-400 text-white' : 'bg-slate-200 hover:bg-slate-300'}`}
                        >
            ...
            <div className="flex flex-col gap-3 mt-6">
                <button type="submit" disabled={isSubmitting}
                    ...
                    {isSubmitting ? 'กําลังส่งข้อมูล...' : 'ยืนยันการนัดหมาย'}
                </button>
                <button type="button" onClick={onPrev} disabled={isSubmitting} className="w-full bg-slate-200 py-4 
text-lg font-bold hover:bg-slate-300 rounded-md">ย้อนกลับ</button>
            </div>
        </form>
    );
}
